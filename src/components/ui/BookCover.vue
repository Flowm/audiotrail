...
<template>
  <div
    ...
    "
  >
    <img v-if="showReal" :src="resolvedUrl!" :alt="`Cover of ${title}`" class="h-full w-full object-cover" loading="lazy" @error="failed = true" @load="onLoad" />
    <span v-else class="font-display absolute inset-0 flex items-center justify-center font-semibold text-white/85" aria-hidden="true">
      {{ titleInitials(title) }}
    </span>
  </div>
</template>

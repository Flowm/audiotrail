...
<template>
  <div class="min-h-screen lg:flex">
    <aside
      class="sticky top-0 hidden h-screen w-60 shrink-0 flex-col border-r border-paper-200 bg-paper-100/60 lg:flex dark:border-ink-800 dark:bg-ink-900/30"
    >
      ...
      <nav class="mt-8 flex-1 space-y-7 overflow-y-auto px-3 pb-4">
        <div v-if="takeout.hasData">
          <p class="overline px-3 pb-2">Explore</p>
        ...
        <div v-else class="px-3">
          <p class="text-xs leading-relaxed text-ink-500 dark:text-ink-400">
            No takeout loaded yet.
          </p>
          <RouterLink
            to="/"
            class="mt-2 inline-block text-xs font-medium text-accent-700 underline-offset-2 hover:underline dark:text-accent-300"
          >
        ...
        <div>
          <p class="overline px-3 pb-2">Story</p>
          <div class="space-y-0.5">
            <RouterLink
      ...
      <div class="space-y-3 border-t border-paper-200 px-5 py-4 dark:border-ink-800">
        ...
        <div class="flex items-center justify-between">
          ...
          <button
            v-if="takeout.hasData"
            type="button"
            class="rounded-lg px-2.5 py-2 text-xs font-medium text-ink-500 transition-colors hover:bg-paper-200/60 hover:text-ink-900 dark:text-ink-400 dark:hover:bg-ink-800/60 dark:hover:text-ink-50"
            @click="clearData"
          >
            Clear data
          </button>
        </div>
        <p class="flex items-center gap-1.5 font-mono text-[9.5px] uppercase tracking-[0.18em] text-ink-400 dark:text-ink-500">
    ...
    <div class="min-w-0 flex-1">
      <header
        class="sticky top-0 z-40 border-b border-paper-200 bg-paper-50/90 backdrop-blur lg:hidden dark:border-ink-800 dark:bg-ink-950/90"
      >
        <div class="flex items-center justify-between px-4 py-3">
          <RouterLink to="/" class="flex items-center gap-2">
            <LogoMark class="h-7 w-7" />
            <span class="font-display text-base font-semibold tracking-tight text-ink-900 dark:text-paper-50">
              Audiotrail
            </span>
          </RouterLink>
          <DarkToggle />
        ...
        <nav
          v-if="takeout.hasData"
          class="flex gap-1 overflow-x-auto px-3 pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
          <RouterLink
            v-for="item in [...explore, wrapped, about]"
            :key="item.to"
            :to="item.to"
            custom
            v-slot="{ href, navigate, isActive }"

...
export function rhythmOption(granularity: RhythmGranularity, days: DayTotal[], p: ChartPalette): EChartsOption {
  ...
  // YYYY-MM strings. Tick at month boundaries instead; an explicit interval
  // disables echarts' own thinning, so hideOverlap has to stand in for it
  // when the brush is zoomed far out.
  const monthOf = (value: string): string => value.slice(0, 7);
  const isMonthTick = (index: number): boolean => {
    const current = categories[index];
    if (current === undefined) return false;
    return index === 0 || monthOf(categories[index - 1]!) !== monthOf(current);
  };
  const axisLabel =
    granularity === "month"
      ? { ...monoAxisLabel(p), formatter: (value: string) => formatMonth(value) }
      : { ...monoAxisLabel(p), interval: isMonthTick, hideOverlap: true, formatter: (value: string) => formatMonth(monthOf(value)) };

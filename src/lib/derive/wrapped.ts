...
export function wrappedStats(
  ...
  const busiestMonth = months.reduce<{ month: string; ms: number } | null>(
    (currentBest, entry) => (currentBest === null || entry.ms > currentBest.ms ? entry : currentBest),
    null,
  )

  const booksStarted = books.filter((book) => book.firstListen?.startsWith(prefix)).length
  const booksFinished = books.filter(
    (book) => book.finished && book.lastListen?.startsWith(prefix),
  ).length
  ...
  const cash = purchases
    .filter(
      (purchase) => purchase.type === 'CASH' && purchase.orderPlaceDate.startsWith(prefix),
    )
    .reduce((sum, purchase) => sum + (purchase.pricePaid ?? 0), 0)
  ...
  const almostFinished = books
    .filter(
      (book) =>
        ...
        book.completion >= 0.85 &&
        book.completion < 0.99 &&
        !book.finished &&
        book.lastListen?.startsWith(prefix),
    )

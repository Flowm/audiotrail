...
export function creditFlow(credits: Credit[]): CreditFlow {
  ...
  return {
    earnedByReason: [...byReason.entries()]
...
export function costPerYear(
  ...
  const hoursByYear = new Map(
    yearlyTotals(days).map((entry) => [entry.year, entry.ms / 3_600_000]),
  )
...
export function creditsSavings(purchases: Purchase[], billings: BillingEvent[]): CreditSavings {
  ...
  const membershipCost = billings
    .filter((billing) => billing.type === 'Charge')

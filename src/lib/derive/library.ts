...
export function backlogStats(books: BookStats[]): BacklogStats {
  const neverListened = books
    .filter(
      (book) =>
        book.totalMs === 0 && book.library !== null && book.library.ownership !== 'Revoked',
    )
 ...
 */
export function acquisitionsByMonth(
  library: LibraryItem[],
  purchases: Purchase[],
): AcquisitionMonth[] {
  ...
  return monthSpan(months[0]!, months[months.length - 1]!).map(
    (month) => byMonth.get(month) ?? { month, credit: 0, cash: 0, other: 0 },
  )
}

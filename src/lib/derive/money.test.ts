...
describe("creditFlow / creditSankeyData", () => {
  ...
  it("produces a balanced sankey (in = out at the center node)", () => {
    ...
    const inflow = sankey.links.filter((link) => link.target === "4 credits").reduce((sum, link) => sum + link.value, 0);
    const outflow = sankey.links.filter((link) => link.source === "4 credits").reduce((sum, link) => sum + link.value, 0);

...
export function buildBookStats(
  ...
  for (const session of sessions) {
    ...
    if (!entry) {
      const matched =
        (session.asin !== null ? libraryByAsin.get(session.asin) : undefined) ??
        libraryByTitle.get(normalizeTitle(session.productName)) ??
        null
  ...
  for (const entry of accumulators.values()) {
    const lengthMs =
      entry.bookLengthMs ??
      (entry.library?.lengthMinutes != null ? entry.library.lengthMinutes * 60_000 : null)
    const completion =
      lengthMs !== null && lengthMs > 0 && entry.maxEndPositionMs !== null
        ? Math.min(1, Math.max(0, entry.maxEndPositionMs / lengthMs))
        : null
    ...
    books.push({
      ...
      completion,
      finished:
        entry.library?.isFinished === true ||
        (completion !== null && completion >= FINISH_THRESHOLD),
    })
  }

...
describe("buildBookStats", () => {
  it("joins by ASIN and accumulates sessions", () => {
    ...
    const { books } = buildBookStats([session({ endPositionMs: 300_000 }), session({ startDate: d("2024-01-03"), durationMs: 120_000, endPositionMs: 540_000 })], lib);
  ...
  it("falls back to product-name matching when ASINs differ", () => {
    ...
    const { books, unmatchedListened } = buildBookStats([session({ asin: "B0PARTASIN", productName: "The  same title " })], lib);
    expect(books).toHaveLength(1);
  ...
  it("uses library length when sessions lack one, and Audible finished flag wins", () => {
    const lib = [libraryItem({ asin: "B000000001", title: "Some Book", lengthMinutes: 100, isFinished: true })];
    const { books } = buildBookStats([session({ bookLengthMs: null, endPositionMs: 3_000_000 })], lib);
    const book = books[0]!;
  ...
  it("counts listened books without any library match", () => {
    const { unmatchedListened } = buildBookStats([session({ asin: "B0NOWHERE1", productName: "Ghost Book" })], [libraryItem({ asin: "B0OTHER", title: "Different" })]);

...
const KID_CSV =
  ...
  [
    HEADER,
    '"2024-02-01","2024-02-01","120000","0","120000","Kids Book","B000TEST03","240000","Download","1.00","0","FullTitle","No","Online","Audible","4.0","Europe/Berlin"',
  ].join("\n");
...
describe("listeningDataset", () => {
  ...
  it("parses sessions, extracts profiles and skips incomplete rows", async () => {
    const result = await listeningDataset.parse([vf("Audible.Listening/Main Profile/Listening.csv", MAIN_CSV), vf("Audible.Listening/Kid/Listening.csv", KID_CSV)]);
  ...
  it("collapses multipart echo rows into one session with the base title", async () => {
    ...
    const result = await listeningDataset.parse([vf("Audible.Listening/Main/Listening.csv", csv)]);

    expect(result.rows).toBe(2);
    expect(result.detail).toContain("2 multipart echo rows dropped");
    expect(result.patch.listening!.every((s) => s.productName === "Epic Saga (Unabridged)")).toBe(true);
  });
});

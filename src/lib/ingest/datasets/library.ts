...
export function mergeLibraryRows(rows: RawLibraryRow[]): {
  ...
  for (const [asin, group] of byAsin) {
    ...
    const purchaseDates = group.map((row) => epochMs(row.purchase_date)).filter((v): v is number => v !== null);
    const series = group.map((row) => parseSeriesInfo(row.book_series_info)).find((refs) => refs.length > 0);

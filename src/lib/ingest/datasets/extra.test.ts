...
describe("purchasesDataset", () => {
  it("parses orders with credit/cash semantics intact", async () => {
    ...
    const result = await purchasesDataset.parse([vf("Audible.PurchaseHistory/Audible.PurchaseHistory.csv", csv)]);
...
describe("billingsDataset", () => {
  it("parses charges and vets", async () => {
    ...
    const result = await billingsDataset.parse([vf("Audible.MembershipBillings/Audible.MembershipBillings.csv", csv)]);
    expect(result.rows).toBe(2);
...
describe("searchHitsDataset", () => {
  it("parses positions and trusts num_* counters over the dead yes/no flags", async () => {
    ...
    const result = await searchHitsDataset.parse([vf("Digital.Audible.SearchData_Tommy_ASIN/Digital.Audible.SearchData_Tommy_ASIN.csv", csv)]);
...
describe("membershipEventsDataset", () => {
  it("collects positive count columns as flags", async () => {
    ...
    const result = await membershipEventsDataset.parse([vf("Audible.MembershipEvent/Audible.MembershipEvent.csv", csv)]);

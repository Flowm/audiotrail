...
const creditChart = computed(() =>
  credits.value.length > 0 ? sankeyOption(creditSankeyData(flow.value), palette.value, (value) => `${formatNumber(value)} credit${value === 1 ? "" : "s"}`) : null,
);

const years = computed(() => costPerYear(billings.value, purchases.value, takeout.days, takeout.bookStats.books));
  ...
  <div class="space-y-10">
    <section>
      ...
      <div class="mt-2 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <h1 class="font-display text-ink-900 dark:text-paper-50 text-3xl font-semibold tracking-tight">Money</h1>
        <p class="text-ink-400 dark:text-ink-500 font-mono text-[10px] tracking-[0.16em] uppercase">all amounts in EUR</p>
      </div>
    </section>

    <template v-if="hasMoney || creditsAvailability.available.value">
      <section class="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard label="Lifetime spend" :value="hasMoney ? formatEur(totalSpend) : '—'" sub="membership + shop" />
        <StatCard label="Cost per hour" :value="lifetimeCostPerHour !== null ? formatEur(lifetimeCostPerHour) : '—'" sub="lifetime average" />
        <StatCard
          label="Credits earned"
          :value="creditsAvailability.available.value ? formatNumber(flow.total) : '—'"
          :sub="creditsAvailability.available.value ? `${formatNumber(flow.consumed)} spent · ${formatNumber(flow.expired)} expired` : null"
        />
        <StatCard
      ...
      <section v-if="expiring.length > 0 || returns.length > 0" class="grid gap-6 lg:grid-cols-2">
        <div v-if="expiring.length > 0" class="space-y-3">
          ...
          <div class="panel divide-paper-200/70 dark:divide-ink-800/70 divide-y px-5">
            <div v-for="credit in expiring" :key="credit.issueDate + (credit.expireDate ?? '')" class="flex items-baseline justify-between gap-4 py-2.5">
              <span class="text-ink-700 dark:text-ink-200 text-sm"> 1 credit · {{ credit.reason ?? "unknown origin" }} </span>
              <span class="font-mono text-xs text-rose-600 dark:text-rose-400"> expires {{ formatDate(credit.expireDate) }} </span>
            </div>
          </div>
        </div>
          ...
          <div class="panel divide-paper-200/70 dark:divide-ink-800/70 divide-y px-5">
            <div v-for="(entry, index) in returns" :key="index" class="py-2.5">
              <div class="flex items-baseline justify-between gap-4">
                <span class="text-ink-700 dark:text-ink-200 min-w-0 flex-1 truncate text-sm" :title="entry.productName ?? undefined">
                  {{ entry.productName ?? "Unknown title" }}
                </span>
                <span class="text-ink-500 dark:text-ink-400 shrink-0 font-mono text-xs">
                  {{ formatDate(entry.returnDate) }}
                </span>
              </div>
              <p class="text-ink-400 dark:text-ink-500 mt-0.5 font-mono text-[11px]">
                <template v-if="entry.creditsRefunded"> {{ entry.creditsRefunded }} credit refunded </template>
    ...
    <EmptyState v-else title="No money data in this takeout" message="Billing, purchase and credit files are all missing — nothing to account for." />
  </div>
</template>

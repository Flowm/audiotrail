...
const memberSince = computed(() => {
  ...
  const firstListen =
    days.value.length > 0 ? Date.parse(`${days.value[0]!.date}T00:00:00Z`) : null
...
function spendIn(yearPrefix: string | null): number {
  ...
  const membership = billings
    .filter((b) => b.type === 'Charge' && (yearPrefix === null || b.billingDate.startsWith(yearPrefix)))
    .reduce((sum, b) => sum + (b.totalAmount ?? 0), 0)
  const cash = purchases
    .filter(
      (p) => p.type === 'CASH' && (yearPrefix === null || p.orderPlaceDate.startsWith(yearPrefix)),
    )
    .reduce((sum, p) => sum + (p.pricePaid ?? 0), 0)
  return membership + cash
}

const hasMoneyData = computed(
  () => (takeout.bundle?.billings.length ?? 0) > 0 || (takeout.bundle?.purchases.length ?? 0) > 0,
)
...
const heatmapOption = computed(() =>
  days.value.length > 0 && selectedYear.value !== null
    ? calendarHeatmapOption(days.value, selectedYear.value, palette.value)
    : null,
)

const cumulativeOption = computed(() =>
  days.value.length > 0 ? cumulativeHoursOption(cumulative(days.value), palette.value) : null,
)

const topBooksOpt = computed(() =>
  books.value.some((book) => book.totalMs > 0)
    ? topBooksOption(books.value, 5, palette.value)
    : null,
)
...
const yoy = computed(() => {
  ...
  const finishedIn = (year: number): number =>
    books.value.filter((book) => book.finished && book.lastListen?.startsWith(`${year}-`)).length
    ...
    <template v-if="listening.available.value">
      <section class="grid grid-cols-2 gap-3 lg:grid-cols-5">
        <StatCard
          ...
          :sub="`across ${formatNumber(days.length)} days with audio`"
        />
        <StatCard
          label="Books started"
        ...
        <StatCard
          ...
          :value="`${formatNumber(streak.days)} days`"
          :sub="streak.start ? `${formatDate(streak.start)} — ${formatDate(streak.end)}` : null"
        />
        <StatCard
          label="Member since"
          :value="memberSince ? formatDate(memberSince) : '—'"
          :sub="takeout.accountInfo?.marketplace ?? null"
        />
        <StatCard
        ...
        <div class="space-y-3 lg:col-span-2">
          ...
          <div class="panel p-3 sm:p-4">
            <BaseChart
              :option="topBooksOpt"
              :height="280"
              empty-title="No full-title listening yet"
            />
          </div>
        </div>
      </section>
      ...
      <section v-if="yoy" class="space-y-3">
        <SectionHeader
          :title="`${yoy.current} vs ${yoy.previous}`"
          :hint="yoy.partial ? `${yoy.current} is still in progress` : 'year over year'"
        />
        <div class="grid gap-3 sm:grid-cols-3">
          <div v-for="row in yoy.rows" :key="row.label" class="panel px-4 py-4 sm:px-5">
            ...
            <div class="mt-1.5 flex flex-wrap items-baseline gap-2">
              <span class="font-display text-2xl font-semibold tracking-tight text-ink-900 dark:text-paper-50">
                {{ row.current }}
              </span>
              <span
                v-if="row.delta"
                :class="[
                  ...
                  row.up
                    ...
                    : 'bg-paper-200/70 text-ink-500 dark:bg-ink-800/70 dark:text-ink-400',
                ]"
              >
                {{ row.delta }}
              </span>
            </div>
            <p class="mt-1 text-xs text-ink-500 dark:text-ink-400">{{ row.note }}</p>
          </div>
        </div>
      </section>

      <p
        v-if="takeout.bookStats.unmatchedListened > 0"
        class="font-mono text-[10px] uppercase tracking-[0.16em] text-ink-400 dark:text-ink-600"
      >
        {{ takeout.bookStats.unmatchedListened }} listened titles had no library match — they're
        counted by product name
      </p>
    </template>

    <EmptyState
      v-else
      title="No listening history in this takeout"
      message="The Audible.Listening folder seems to be missing or empty, so the overview has nothing to chart."
    />
  </div>
</template>

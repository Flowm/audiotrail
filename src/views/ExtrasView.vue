...
const wishlistFunnel = computed(() => {
  ...
  const deleted = wishlist.value.filter(
    (w) => w.status === 'Wishlist Item Deleted' || (w.deleteDate !== null && w.status !== 'Wishlist Item Purchased'),
  ).length
...
const survivors = computed(() =>
  wishlist.value
    .filter((w) => w.status === 'Wishlist Item Added' && w.deleteDate === null)
...
const survivorAges = computed(() => {
  const now = Date.now()
  return survivors.value
    .map((w) => Math.floor((now - Date.parse(`${w.addDate}T00:00:00Z`)) / DAY_MS))
    .sort((a, b) => a - b)
})
...
const userCollections = computed(() => {
  ...
  return infos.map((info) => ({
    info,
    items: collectionItems.value.filter(
      (item) => item.collectionId === info.id && item.isDeleted !== true,
    ),
  }))
})
...
const anythingAvailable = computed(
  () =>
    wishlistAvailability.available.value ||
    searchAvailability.available.value ||
    devicesAvailability.available.value ||
    collectionsAvailability.available.value,
)
</script>
...
<template>
  <div class="space-y-10">
    <section>
      ...
      <h1 class="mt-2 font-display text-3xl font-semibold tracking-tight text-ink-900 dark:text-paper-50">
        Extras
      </h1>
    </section>
    ...
    <template v-if="anythingAvailable">
      ...
      <section v-if="wishlistAvailability.available.value" class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <StatCard
            label="Still waiting"
            :value="formatNumber(wishlistFunnel.waiting)"
            :sub="medianWishAge !== null ? `median age ${formatNumber(medianWishAge)} days` : null"
          />
        </div>
        <div v-if="survivors.length > 0" class="panel px-5 py-4">
          <p class="overline pb-3">Longest-waiting wishes</p>
          <div class="grid gap-x-8 gap-y-1.5 sm:grid-cols-2">
            <div
              v-for="wish in survivors.slice(0, 10)"
              :key="wish.asin ?? wish.addDate"
              class="flex items-baseline justify-between gap-3 text-sm"
            >
              <span class="min-w-0 flex-1 truncate text-ink-700 dark:text-ink-200" :title="wish.productName ?? undefined">
                {{ wish.productName ?? 'Unknown title' }}
              </span>
              <span class="shrink-0 font-mono text-[11px] text-ink-400 dark:text-ink-500">
                since {{ formatDate(wish.addDate) }}
              </span>
            </div>
          </div>
        </div>
      ...
      <section v-if="searchAvailability.available.value" class="space-y-3">
        ...
        <div class="grid grid-cols-3 gap-3">
          ...
          <StatCard
            label="Led to a click"
            :value="searchTotals.total > 0 ? formatPercent(searchTotals.clicked / searchTotals.total) : '—'"
          />
          <StatCard
            label="Led to an order"
            :value="searchTotals.total > 0 ? formatPercent(searchTotals.purchased / searchTotals.total) : '—'"
          />
        </div>
        <div class="grid gap-3 lg:grid-cols-2">
          <div class="panel px-5 py-4">
            <p class="overline pb-3">What you searched for</p>
            <div class="flex flex-wrap gap-1.5">
              <span
                v-for="entry in topKeywords"
                :key="entry.keyword"
                class="rounded-full border border-paper-200 bg-white/60 px-2.5 py-1 text-xs text-ink-600 dark:border-ink-800 dark:bg-ink-900/60 dark:text-ink-300"
              >
                {{ entry.keyword }}
                <span class="font-mono text-[10px] text-ink-400 dark:text-ink-500">×{{ entry.searches }}</span>
              </span>
            </div>
          </div>
          <div v-if="hitsAvailability.available.value" class="panel p-3 sm:p-4">
            <p class="overline px-2 pt-1 pb-2">Which result you click</p>
      ...
      <section v-if="devicesAvailability.available.value || playbackAvailability.available.value" class="space-y-3">
        ...
        <div class="grid gap-3 lg:grid-cols-2">
          <div v-if="devices.length > 0" class="panel px-5 py-4">
            <p class="overline pb-3">Device activations</p>
            <div class="space-y-2">
              <div v-for="(device, index) in devices" :key="index" class="flex items-baseline justify-between gap-3 text-sm">
                <span class="min-w-0 flex-1 truncate text-ink-700 dark:text-ink-200">
          ...
          <div v-if="outputShare.length > 0" class="panel p-3 sm:p-4">
            <p class="overline px-2 pt-1">
              Where the sound goes
              <span v-if="playbackRange" class="normal-case tracking-normal">
                · {{ formatDate(playbackRange.from) }} – {{ formatDate(playbackRange.to) }} sample only
              </span>
            </p>
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <div v-for="entry in userCollections" :key="entry.info.id" class="panel px-5 py-4">
            <div class="flex items-baseline justify-between gap-3">
              <p class="font-display text-base font-semibold text-ink-900 dark:text-paper-50">
                {{ entry.info.name ?? 'Untitled collection' }}
              </p>
              <span class="font-mono text-[11px] text-ink-400 dark:text-ink-500">
                {{ entry.items.length }} titles
              </span>
            </div>
            <p v-if="entry.info.description" class="mt-1 text-xs text-ink-500 dark:text-ink-400">
              {{ entry.info.description }}
            </p>
            <p class="mt-2 line-clamp-2 text-xs leading-relaxed text-ink-500 dark:text-ink-400">
              {{ entry.items.map((item) => item.productName).filter(Boolean).join(' · ') }}
            </p>
          </div>
        </div>
      </section>
    ...
    <EmptyState
      v-else
      title="No extras in this takeout"
      message="Wishlist, search, device and collection files are all missing."
    />
  </div>
</template>

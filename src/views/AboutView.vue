...
<template>
  <div class="space-y-14">
    <section>
      ...
      <h1 class="mt-2 font-display text-3xl font-semibold tracking-tight text-ink-900 sm:text-4xl dark:text-paper-50">
        A private window into your listening
      </h1>
      <p class="mt-5 max-w-2xl text-[15px] leading-relaxed text-ink-600 dark:text-ink-300">
        Audible's GDPR data takeout is a zip full of CSV files — technically yours, practically
        unreadable. Audiotrail turns it into something you can actually explore: your listening
        rhythm over the years, the books you abandoned at 90%, what a listening hour really cost
        you. It is a static web page with no backend; the analysis runs entirely on your machine.
      </p>
    </section>

    <section>
      <p class="overline">Privacy, in concrete terms</p>
      <ul class="mt-4 max-w-2xl space-y-3 text-sm leading-relaxed text-ink-600 dark:text-ink-300">
        <li class="flex gap-3">
          <span class="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-accent-500" />
          Your zip is read and parsed by JavaScript inside this tab. No byte of it is ever
          uploaded — there is no server to upload to.
        </li>
        <li class="flex gap-3">
          <span class="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-accent-500" />
          No analytics, no cookies. The browser's localStorage holds only display preferences
          (dark mode, the cover-art toggle) — never your data.
        </li>
        <li class="flex gap-3">
          <span class="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-accent-500" />
        ...
        <li class="flex gap-3">
          <span class="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-accent-500" />
          One deliberate exception: an opt-in toggle (off by default) can load real cover art.
          Because Audible artwork can't be addressed by ASIN directly, enabling it sends each
          book's ASIN to <a href="https://audnex.us" target="_blank" rel="noopener" class="underline underline-offset-2">Audnexus</a>
          (a community Audible-metadata API) to look up the cover, which then loads from Amazon's
          image CDN — so both Audnexus and Amazon see your library's book IDs. Leave it off and
          you get generated placeholder covers instead.
        </li>
      </ul>
    </section>
    ...
    <section>
      ...
      <dl class="mt-4 grid gap-x-8 gap-y-4 sm:grid-cols-2">
        <div v-for="dataset in datasets" :key="dataset.name">
          <dt class="text-sm font-semibold text-ink-800 dark:text-ink-100">{{ dataset.name }}</dt>
          <dd class="mt-0.5 text-[13px] leading-relaxed text-ink-500 dark:text-ink-400">
            {{ dataset.blurb }}
          </dd>
        </div>
      </dl>
    ...
    <section>
      ...
      <p class="mt-4 max-w-2xl text-sm leading-relaxed text-ink-600 dark:text-ink-300">
        Built with Vue 3, Vite, Tailwind CSS and Apache ECharts; zips are opened with JSZip and
        CSVs parsed with Papa Parse. Type is set in Fraunces, Schibsted Grotesk and IBM Plex Mono,
        self-hosted so the app makes no font-CDN requests. Opt-in cover art is resolved through
        the Audnexus API. Audiotrail is an independent project and is not affiliated with, endorsed
        by, or connected to Audible or Amazon; all trademarks belong to their owners.
      </p>
    </section>
  </div>
</template>
